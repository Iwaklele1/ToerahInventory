...
const InventoryPage = () => {
  ...
  const getProductChanges = (
    ...
    if ((oldItem.description ?? "") !== (newItem.description ?? ""))
      changes.push(
        `Deskripsi: ${oldItem.description || "-"} ➝ ${
          newItem.description || "-"
        }`
      );
    if ((oldItem.image ?? "") !== (newItem.image ?? ""))
      changes.push(`Gambar: diubah`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditMode && editingId !== null) {
      ...
      await sendTelegramNotification(
        `✏️ <b>${escapeHtml(username)}</b> mengedit produk: <b>${escapeHtml(
  ...
  return (
    <div className="inventory-main">
      ...
      <div className="manage-inventory">
        <div className="inventory-header-form">
          <h3>{isEditMode ? "Edit Product" : "Add New Product"}</h3>
        </div>
        <div className="inventory-body-form">
          <form onSubmit={handleSubmit}>
            ...
            <div className="quantity-control">
              <button
                className="quantity-control left"
                type="button"
                onClick={() =>
                  setNewProduct({
                    ...newProduct,
              ...
              <button
                className="quantity-control right"
                type="button"
                onClick={() =>
                  setNewProduct({
                    ...newProduct,
